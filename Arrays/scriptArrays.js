...
export const arrays = () => {
  ...
  const vievMethodContent = (dataArray) => {
    let element = "";
    let [, inputValue, spec, content, additionalContents] = dataArray;
      ...
      const methodsArraySettings = (name, objects, inputType, spec) => {
        ...
        if (spec === "methods") {
          objects.forEach((obj) => {
            element += `
            ${obj.active ?
                (name === "reduce" ? (obj.methodContent).trim() + ",&nbsp;" : obj.methodContent.replaceAll("'", "\""))
                : ""}
            `;
          });
        } else {
        ...
        element += `
          ${name === "editing an element of an array" ? `
            ...
            <label for="editIndex" class="range--label">
              index of edit:
              <span class="js-editIndex">${editIndex}</span>
            </label>
          ` : ""} 
          ${name === "removing an element from an array" ? `
            ...
            <label for="removeIndex" class="range--label">
              index of remove:
              <span class="js-removeIndex">${removeIndex}</span>
            </label>
          ` : ""} 
        `;
      ...
      formElement.addEventListener("submit", (event) => {
              ...
              if (method.method === button.id) {
                ...
                if (pattern.test(input.value)) {
                  if (checkDependency_1(method) || checkDependency_2(method, input)) {
                    ...
                    runMethod(button.id, enterNumberOrString(input.value), method.method, method.spec, method.contents, method.additionalContents);
        ...
        methodsArray.forEach((method) => {
          if ((method.method === button.id) && [null, "range"].includes(method.inputType)) {
            runMethod(button.id, null, method.method, method.spec, method.contents, method.additionalContents)
            render();
          };
        });
      });
        ...
        methodsArray.forEach((object) => {
          if (object.method === button.name) {
            ...
            (object.spec === "immutability") ?
              outputInfo = `You have chosen to create a new array using the spread syntax. <br> 
              The new array will contain all the elements of the "array", 
              with the added element ${activeButton.replace("[...array,", "").replace("]", "").replaceAll("'", "\"")} at the end. <br>
              Now click the "Run" button.`
              :
  ...
  const runMethod = (button, inputValue, method, spec, content, additionalContents) => {
    outputInfo = "";
